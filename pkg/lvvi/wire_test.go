...
func TestTreeEndpointsRejectsUnknownTopologies(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			"4-Y comb (byte0=10)",
			[]byte{
				0x0a, 0x00, 0x08, 0x07, 0x06, 0x00, 0x03, 0x03, 0x05, 0x00, 0x03, 0x41,
				0x21, 0x4a, 0x59, 0x59, 0x25, 0x48, 0x55, 0x57,
			},
		},
		{
			"3-Y comb (byte0=10)",
			[]byte{
				0x0a, 0x00, 0x08, 0x00, 0x06, 0x00, 0x06, 0x00, 0x00, 0x03, 0x03, 0x41,
...
func TestHeapWireCombWithElbowBaseline(t *testing.T) {
	want := []byte{
		0x0a, 0x00, 0x08, 0x00, 0x06, 0x00, 0x06, 0x00,
