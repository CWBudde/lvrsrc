...
func emit(out io.Writer, specs []*enumSpec) error {
	if err := writef(out, "// Code generated by scripts/gen-heap-tags. DO NOT EDIT.\n"); err != nil {
		return err
	}
	if err := writef(out, "// Source: references/pylabview/pylabview/LVheap.py\n"); err != nil {
		return err
	}
	if err := writef(out, "// Generated: %s\n\n", time.Now().UTC().Format(time.RFC3339)); err != nil {
		return err
	}
	if err := writef(out, "package heap\n\n"); err != nil {
		return err
	}
	if err := writef(out, "import \"fmt\"\n\n"); err != nil {
		return err
	}

	for _, s := range specs {
		if err := writef(out, "// %s mirrors pylabview's %s enum.\n", s.goType, s.pyName); err != nil {
			return err
		}
		if err := writef(out, "type %s int\n\n", s.goType); err != nil {
			return err
		}
		if len(s.members) == 0 {
			if err := writef(out, "// %s has no documented members in pylabview.\n\n", s.goType); err != nil {
				return err
			}
			continue
		}
		...
		if err := writef(out, "// Members of %s.\n", s.goType); err != nil {
			return err
		}
		if err := writef(out, "const (\n"); err != nil {
			return err
		}
		for _, m := range s.members {
			constName := s.goType + m.goName
			if err := writef(out, "\t%s %s = %d // %s\n", constName, s.goType, m.value, m.pyName); err != nil {
				return err
			}
		}
		if err := writef(out, ")\n\n"); err != nil {
		...
		if err := writef(out, "var %sNames = map[%s]string{\n", lowerFirst(s.goType), s.goType); err != nil {
			return err
		}
		...
		for _, m := range sorted {
			...
			constName := s.goType + m.goName
			if err := writef(out, "\t%s: %q,\n", constName, canonical[m.value]); err != nil {
				return err
			}
		}
		if err := writef(out, "}\n\n"); err != nil {
		...
		if err := writef(out, "// String returns the pylabview name when known, otherwise a `%s(N)` fallback.\n", s.goType); err != nil {
			return err
		}
		if err := writef(out, "func (t %s) String() string {\n", s.goType); err != nil {
			return err
		}
		if err := writef(out, "\tif name, ok := %sNames[t]; ok {\n\t\treturn name\n\t}\n", lowerFirst(s.goType)); err != nil {
			return err
		}
		if err := writef(out, "\treturn fmt.Sprintf(%q, int(t))\n", s.goType+"(%d)"); err != nil {
			return err
		}
		if err := writef(out, "}\n\n"); err != nil {
			return err
...
func main() {
	...
	if closeErr != nil {
		stderrf("close LVheap.py: %v", closeErr)
		os.Exit(1)
	}
	resolveTypeCollisions(specs)
	disambiguateGoNames(specs)
	if len(specs) == 0 {
		stderrf("no enums extracted")
		os.Exit(1)
	}
	if err := emit(os.Stdout, specs); err != nil {
		stderrf("emit: %v", err)
		os.Exit(1)
	}
	stderrf("extracted %d enums", len(specs))
}
