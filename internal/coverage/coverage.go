...
var semanticDispositions = map[string]Disposition{
	...
	"BDHb": {
		Status:     "partial",
		Semantic:   []string{"heap envelope header", "tag-stream node structure", "tag enum names", "OF__bounds", "OF__termBounds", "OF__termHotPoint", "selected compressed-wire projections"},
		Compressed: []string{"zlib-compressed heap stream preserved byte-for-byte when possible"},
		Opaque:     []string{"per-class primitive fields", "structure metadata", "multi-elbow/manual/comb wire records", "unknown tags surfaced as Tag(N)"},
	...
	"FPHb": {
		Status:     "partial",
		Semantic:   []string{"heap envelope header", "tag-stream node structure", "tag enum names", "OF__bounds", "selected numeric data fills"},
		Compressed: []string{"zlib-compressed heap stream preserved byte-for-byte when possible"},
		Opaque:     []string{"per-class visual fields", "label/caption/font/color/style records", "many rectangle-like fields", "custom-control state", "unknown tags surfaced as Tag(N)"},
