...
func TestBuildManifestFromCorpus(t *testing.T) {
	...
	if m.Corpus.FixtureCount != 68 {
		t.Fatalf("Corpus.FixtureCount = %d, want 68", m.Corpus.FixtureCount)
	}
	...
	if got, want := m.Corpus.Breadth.FileKinds["vi"], 57; got != want {
	...
	if got, want := m.Corpus.Breadth.FormatVersions["3"], 68; got != want {
		t.Fatalf("Breadth.FormatVersions[3] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.LabVIEWVersions["25.3.2"], 55; got != want {
		t.Fatalf("Breadth.LabVIEWVersions[25.3.2] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.Platforms["unknown"], 68; got != want {
		t.Fatalf("Breadth.Platforms[unknown] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.TextEncodings["unknown"], 68; got != want {
		t.Fatalf("Breadth.TextEncodings[unknown] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.PasswordProtection["empty-password"], 57; got != want {
	...
	if got, want := m.Corpus.Breadth.LockedFlags["false"], 68; got != want {
		t.Fatalf("Breadth.LockedFlags[false] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.SeparateCompiledCode["true"], 68; got != want {
	...
	if mono.CorpusFixtures != 68 {
		t.Fatalf("ICON CorpusFixtures = %d, want 68", mono.CorpusFixtures)
	}
	if mono.CorpusSections != 68 || mono.CorpusBytes != 8704 {
		t.Fatalf("ICON corpus totals = sections %d bytes %d, want sections 68 bytes 8704", mono.CorpusSections, mono.CorpusBytes)
	}
	...
	if conp.CorpusFixtures != 68 {
		t.Fatalf("CONP CorpusFixtures = %d, want 68", conp.CorpusFixtures)
	}
	...
	if cpc2.CorpusFixtures != 68 {
		t.Fatalf("CPC2 CorpusFixtures = %d, want 68", cpc2.CorpusFixtures)
	}
	...
	if lifp.CorpusFixtures != 68 {
		t.Fatalf("LIfp CorpusFixtures = %d, want 68", lifp.CorpusFixtures)
	}
	...
	if libd.CorpusFixtures != 68 {
		t.Fatalf("LIbd CorpusFixtures = %d, want 68", libd.CorpusFixtures)
	}
	...
	if vers.CorpusFixtures != 68 {
		t.Fatalf("vers CorpusFixtures = %d, want 68", vers.CorpusFixtures)
	}
	...
	if vctp.CorpusFixtures != 68 {
		t.Fatalf("VCTP CorpusFixtures = %d, want 68", vctp.CorpusFixtures)
	}
	...
	if bdpw.CorpusFixtures != 57 {
		t.Fatalf("BDPW CorpusFixtures = %d, want 57", bdpw.CorpusFixtures)
	}
	...
	if bdhb.CorpusFixtures != 68 {
		t.Fatalf("BDHb CorpusFixtures = %d, want 68", bdhb.CorpusFixtures)
	}
