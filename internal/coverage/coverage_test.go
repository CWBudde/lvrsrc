...
func TestBuildManifestFromCorpus(t *testing.T) {
	...
	if m.Corpus.FixtureCount != 49 {
		t.Fatalf("Corpus.FixtureCount = %d, want 49", m.Corpus.FixtureCount)
	}
	...
	if got, want := m.Corpus.Breadth.FileKinds["vi"], 38; got != want {
	...
	if got, want := m.Corpus.Breadth.FormatVersions["3"], 49; got != want {
		t.Fatalf("Breadth.FormatVersions[3] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.LabVIEWVersions["25.3.2"], 36; got != want {
		t.Fatalf("Breadth.LabVIEWVersions[25.3.2] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.Platforms["unknown"], 49; got != want {
		t.Fatalf("Breadth.Platforms[unknown] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.TextEncodings["unknown"], 49; got != want {
		t.Fatalf("Breadth.TextEncodings[unknown] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.PasswordProtection["empty-password"], 38; got != want {
	...
	if got, want := m.Corpus.Breadth.LockedFlags["false"], 49; got != want {
		t.Fatalf("Breadth.LockedFlags[false] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.SeparateCompiledCode["true"], 49; got != want {
	...
	if mono.CorpusFixtures != 49 {
		t.Fatalf("ICON CorpusFixtures = %d, want 49",mono.CorpusFixtures)
	}
	if mono.CorpusSections != 49 || mono.CorpusBytes != 6272 {
		t.Fatalf("ICON corpus totals = sections %d bytes %d, want sections 49 bytes 6272", mono.CorpusSections, mono.CorpusBytes)
	}
	...
	if conp.CorpusFixtures != 49 {
		t.Fatalf("CONP CorpusFixtures = %d, want 49",conp.CorpusFixtures)
	}
	...
	if cpc2.CorpusFixtures != 49 {
		t.Fatalf("CPC2 CorpusFixtures = %d, want 49",cpc2.CorpusFixtures)
	}
	...
	if lifp.CorpusFixtures != 49 {
		t.Fatalf("LIfp CorpusFixtures = %d, want 49",lifp.CorpusFixtures)
	}
	...
	if libd.CorpusFixtures != 49 {
		t.Fatalf("LIbd CorpusFixtures = %d, want 49",libd.CorpusFixtures)
	}
	...
	if vers.CorpusFixtures != 49 {
		t.Fatalf("vers CorpusFixtures = %d, want 49",vers.CorpusFixtures)
	}
	...
	if vctp.CorpusFixtures != 49 {
		t.Fatalf("VCTP CorpusFixtures = %d, want 49",vctp.CorpusFixtures)
	}
	...
	if bdpw.CorpusFixtures != 38 {
		t.Fatalf("BDPW CorpusFixtures = %d, want 38", bdpw.CorpusFixtures)
	}
	...
	if bdhb.CorpusFixtures != 49 {
		t.Fatalf("BDHb CorpusFixtures = %d, want 49",bdhb.CorpusFixtures)
	}
