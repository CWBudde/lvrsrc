...
func TestBuildManifestFromCorpus(t *testing.T) {
	...
	if m.Corpus.FixtureCount != 65 {
		t.Fatalf("Corpus.FixtureCount = %d, want 65", m.Corpus.FixtureCount)
	}
	...
	if got, want := m.Corpus.Breadth.FileKinds["vi"], 54; got != want {
	...
	if got, want := m.Corpus.Breadth.FormatVersions["3"], 65; got != want {
		t.Fatalf("Breadth.FormatVersions[3] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.LabVIEWVersions["25.3.2"], 52; got != want {
		t.Fatalf("Breadth.LabVIEWVersions[25.3.2] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.Platforms["unknown"], 65; got != want {
		t.Fatalf("Breadth.Platforms[unknown] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.TextEncodings["unknown"], 65; got != want {
		t.Fatalf("Breadth.TextEncodings[unknown] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.PasswordProtection["empty-password"], 54; got != want {
	...
	if got, want := m.Corpus.Breadth.LockedFlags["false"], 65; got != want {
		t.Fatalf("Breadth.LockedFlags[false] = %d, want %d", got, want)
	}
	if got, want := m.Corpus.Breadth.SeparateCompiledCode["true"], 65; got != want {
	...
	if mono.CorpusFixtures != 65 {
		t.Fatalf("ICON CorpusFixtures = %d, want 65", mono.CorpusFixtures)
	}
	if mono.CorpusSections != 65 || mono.CorpusBytes != 8320 {
		t.Fatalf("ICON corpus totals = sections %d bytes %d, want sections 65 bytes 8320", mono.CorpusSections, mono.CorpusBytes)
	}
	...
	if conp.CorpusFixtures != 65 {
		t.Fatalf("CONP CorpusFixtures = %d, want 65", conp.CorpusFixtures)
	}
	...
	if cpc2.CorpusFixtures != 65 {
		t.Fatalf("CPC2 CorpusFixtures = %d, want 65", cpc2.CorpusFixtures)
	}
	...
	if lifp.CorpusFixtures != 65 {
		t.Fatalf("LIfp CorpusFixtures = %d, want 65", lifp.CorpusFixtures)
	}
	...
	if libd.CorpusFixtures != 65 {
		t.Fatalf("LIbd CorpusFixtures = %d, want 65", libd.CorpusFixtures)
	}
	...
	if vers.CorpusFixtures != 65 {
		t.Fatalf("vers CorpusFixtures = %d, want 65", vers.CorpusFixtures)
	}
	...
	if vctp.CorpusFixtures != 65 {
		t.Fatalf("VCTP CorpusFixtures = %d, want 65", vctp.CorpusFixtures)
	}
	...
	if bdpw.CorpusFixtures != 54 {
		t.Fatalf("BDPW CorpusFixtures = %d, want 54", bdpw.CorpusFixtures)
	}
	...
	if bdhb.CorpusFixtures != 65 {
		t.Fatalf("BDHb CorpusFixtures = %d, want 65", bdhb.CorpusFixtures)
	}
