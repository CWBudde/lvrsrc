...
func TestRoundTripZeroSentinel(t *testing.T) {
	...
	val := v.(Value)
	if val.Flags != 0 {
		t.Errorf("Flags = %#x, want 0", val.Flags)
	}
...
func TestCorpusRoundTrip(t *testing.T) {
	...
	if total == 0 {
		t.Skip("no VPDP sections in corpus")
	}
	t.Logf("exercised %d VPDP section(s); %d had non-zero Flags (corpus expectation: 0)", total, nonZero)
}
