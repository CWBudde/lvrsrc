// Package bdse implements the codec for the "BDSE" resource — Block
// Diagram Size Estimate, a 4-byte big-endian uint32 hint for the
// in-memory size of the block-diagram object graph.
//
// References: pylabview LVblock.py:1383 (BDSE class, SingleIntBlock).
//
// See docs/resources/bdse.md for layout and semantics.
package bdse
...
const FourCC codecs.FourCC = "BDSE"

const payloadSize = 4

// Value is the decoded form of a BDSE payload.
type Value struct {
	// Estimate is the LabVIEW-computed block-diagram size hint, in bytes.
	Estimate uint32
}

// Codec implements codecs.ResourceCodec for the BDSE resource.
type Codec struct{}
...
func (Codec) Decode(_ codecs.Context, payload []byte) (any, error) {
	if len(payload) != payloadSize {
		return nil, fmt.Errorf("BDSE: payload size = %d, want %d", len(payload), payloadSize)
	}
...
func (Codec) Encode(_ codecs.Context, value any) ([]byte, error) {
	...
	case *Value:
		if tv == nil {
			return nil, fmt.Errorf("BDSE: Encode received nil *Value")
		}
		v = *tv
	default:
		return nil, fmt.Errorf("BDSE: Encode expected Value or *Value, got %T", value)
	}
...
func (Codec) Validate(_ codecs.Context, payload []byte) []validate.Issue {
	...
	return []validate.Issue{{
		...
		Code:     "bdse.payload.size",
		Message:  fmt.Sprintf("BDSE payload is %d bytes, want %d", len(payload), payloadSize),
