// Package libd implements a narrow codec for the "LIbd" resource, which
// carries block-diagram metadata/import references.
//
...
// Value is the decoded form of an LIbd payload.
type Value struct {
	Version    uint16
	Marker     string
	EntryCount uint32
...
// Entry is one block-diagram metadata/import reference.
type Entry struct {
	Kind           uint16
	LinkType       string
...
// Codec implements codecs.ResourceCodec for LIbd.
type Codec struct{}
...
func (Codec) Encode(_ codecs.Context, value any) ([]byte, error) {
	...
	if v.Marker != expectedMarker {
		return nil, fmt.Errorf("LIbd: Marker = %q, want %q", v.Marker, expectedMarker)
	}
	if v.EntryCount != uint32(len(v.Entries)) {
		return nil, fmt.Errorf("LIbd: EntryCount = %d, want %d entries", v.EntryCount, len(v.Entries))
	}
	...
	for i, entry := range v.Entries {
		if entry.QualifierCount != uint32(len(entry.Qualifiers)) {
			return nil, fmt.Errorf("LIbd: entry %d QualifierCount = %d, want %d qualifiers", i, entry.QualifierCount, len(entry.Qualifiers))
		}
		out = appendU16(out, entry.Kind)
		if len(entry.LinkType) != 4 {
			return nil, fmt.Errorf("LIbd: entry %d LinkType length = %d, want 4", i, len(entry.LinkType))
		}
		...
		for j, qualifier := range entry.Qualifiers {
			if len(qualifier) > 0xff {
				return nil, fmt.Errorf("LIbd: entry %d qualifier %d length = %d, exceeds 255", i, j, len(qualifier))
			}
...
func (Codec) Validate(_ codecs.Context, payload []byte) []validate.Issue {
	loc := validate.IssueLocation{Area: "LIbd", BlockType: string(FourCC)}
	if len(payload) < headerSize+footerSize {
		return []validate.Issue{{
			Severity: validate.SeverityError,
			Code:     "libd.payload.short",
			Message:  fmt.Sprintf("LIbd payload is %d bytes, need at least %d", len(payload), headerSize+footerSize),
			Location: loc,
		}}
	}
	if string(payload[2:6]) != expectedMarker {
		return []validate.Issue{{
			Severity: validate.SeverityError,
			Code:     "libd.marker.invalid",
			Message:  fmt.Sprintf("LIbd marker = %q, want %q", string(payload[2:6]), expectedMarker),
			Location: loc,
		}}
	}
...
func coerceValue(value any) (Value, error) {
	...
	case *Value:
		if tv == nil {
			return Value{}, fmt.Errorf("LIbd: Encode received nil *Value")
		}
		return *tv, nil
	default:
		return Value{}, fmt.Errorf("LIbd: Encode expected Value or *Value, got %T", value)
	}
}

func decodeValue(payload []byte, strictMarker bool) (Value, error) {
	if len(payload) < headerSize+footerSize {
		return Value{}, fmt.Errorf("LIbd: payload too short: %d bytes (need at least %d)", len(payload), headerSize+footerSize)
	}
	...
	if strictMarker && v.Marker != expectedMarker {
		return Value{}, fmt.Errorf("LIbd: marker = %q, want %q", v.Marker, expectedMarker)
	}
	...
	if consumed != len(rest) {
		return Value{}, fmt.Errorf("LIbd: trailing payload size = %d", len(rest)-consumed)
	}
	v.Entries = entries
	v.Footer = footer
	return v, nil
...
func decodeEntries(payload []byte, remaining int, memo map[parseKey]parseResult) ([]Entry, uint16, int, error) {
	...
	if remaining == 0 {
		if len(payload) != footerSize {
			result = parseResult{
				err:      fmt.Errorf("LIbd: footer size = %d, want %d", len(payload), footerSize),
				recorded: true,
			}
			memo[key] = result
...
func encodePathRef(path PathRef, label string) ([]byte, error) {
	if len(path.Class) != 4 {
		return nil, fmt.Errorf("LIbd: %s class length = %d, want 4", label, len(path.Class))
	}
	if len(path.Raw) == 0 {
	...
	if string(path.Raw[:4]) != path.Class {
		return nil, fmt.Errorf("LIbd: %s raw class = %q, want %q", label, string(path.Raw[:4]), path.Class)
	}
	if binary.BigEndian.Uint32(path.Raw[4:8]) != path.DeclaredLen {
		return nil, fmt.Errorf("LIbd: %s raw declared length = %d, want %d", label, binary.BigEndian.Uint32(path.Raw[4:8]), path.DeclaredLen)
	}
