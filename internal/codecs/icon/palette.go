...
// Palette16 is the classic 16-colour LabVIEW palette used by the `icl4`
// resource. Entries are packed ARGB: bits 24..31 alpha, 16..23 red, 8..15
// green, 0..7 blue. Alpha is always 0xFF (opaque).
//
// Source: references/pylabview/pylabview/LVmisc.py:88-91
// (LABVIEW_COLOR_PALETTE_16).
var Palette16 = [16]uint32{
	0xFFFFFFFF, 0xFFFFFF00, 0xFF000080, 0xFFFF0000,
	0xFFFF00FF, 0xFF800080, 0xFF0000FF, 0xFF00FFFF,
	0xFF00FF00, 0xFF008000, 0xFF800000, 0xFF808000,
	0xFFC0C0C0, 0xFF808080, 0xFF008080, 0xFF000000,
}

// Palette256 is the classic 256-colour LabVIEW palette used by the `icl8`
// resource. Same packed-ARGB shape as Palette16.
//
// Source: references/pylabview/pylabview/LVmisc.py:52-85
// (LABVIEW_COLOR_PALETTE_256).
var Palette256 = [256]uint32{
	0xFFF1F1F1, 0xFFFFFFCC, 0xFFFFFF99, 0xFFFFFF66, 0xFFFFFF33, 0xFFFFFF00, 0xFFFFCCFF, 0xFFFFCCCC,
	...
	0xFF006699, 0xFF006666, 0xFF006633, 0xFF006600, 0xFF3003FF, 0xFF0033CC, 0xFF003399, 0xFF003366,
...
// Palette2 is the trivial 2-colour palette used to colour the 1-bit `ICON`
// resource: index 0 = white, index 1 = black. Exposed so callers that want a
// single RGBA pipeline for all icon depths can treat mono symmetrically.
//
// Source: references/pylabview/pylabview/LVmisc.py:93-95
// (LABVIEW_COLOR_PALETTE_2).
var Palette2 = [2]uint32{
	0xFFFFFFFF, 0xFF000000,
}

// paletteFor returns a freshly allocated slice copy of the palette appropriate
// for the given bit depth. The copy avoids accidental cross-Value aliasing
// when callers mutate the slice (e.g. tests).
func paletteFor(bitsPerPixel int) []uint32 {
	switch bitsPerPixel {
	case 1:
		out := make([]uint32, len(Palette2))
		copy(out, Palette2[:])
		return out
	case 4:
		out := make([]uint32, len(Palette16))
		copy(out, Palette16[:])
		return out
	case 8:
		out := make([]uint32, len(Palette256))
		copy(out, Palette256[:])
		return out
	default:
		return nil
