...
func TestEncodeRoundTripCorpusObservedSizes(t *testing.T) {
	cases := []string{
		...
		"000000020000000000000000", // 12 bytes — count 2
	}
...
func TestDecodeRejectsCountSizeMismatch(t *testing.T) {
	if _, err := (Codec{}).Decode(codecs.Context{}, []byte{0, 0, 0, 5, 0, 0, 0, 0}); err == nil {
...
func TestCorpusRoundTrip(t *testing.T) {
	...
	if total == 0 {
		t.Skip("no BDEx sections in corpus")
	}
	t.Logf("exercised %d BDEx section(s)", total)
}
