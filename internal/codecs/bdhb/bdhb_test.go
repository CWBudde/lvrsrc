		...
		for _, block := range f.Blocks {
			...
			for _, section := range block.Sections {
				...
				if len(v.Tree.Flat) == 0 {
					t.Errorf("%s BDHb has %d-byte content but empty Tree.Flat", e.Name(), v.Envelope.ContentLen)
				}
				return
			}
		}
	}
	t.Skip("no BDHb sections in corpus")
}
...
func TestEncodeRoundTripCorpus(t *testing.T) {
		...
		for _, block := range f.Blocks {
			...
			for _, section := range block.Sections {
				...
				if !bytes.Equal(back, section.Payload) {
					t.Fatalf("%s BDHb id=%d round-trip mismatch (orig %d bytes, re-encoded %d bytes)",
						e.Name(), section.Index, len(section.Payload), len(back))
				}
				issues := Codec{}.Validate(codecs.Context{}, section.Payload)
				if len(issues) != 0 {
					t.Errorf("%s BDHb id=%d Validate issues: %+v", e.Name(), section.Index, issues)
				}
			}
		}
	}
	...
	t.Logf("BDHb codec round-tripped %d corpus sections (%d total tag entries)", totalSections, totalEntries)
}
