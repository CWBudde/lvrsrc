...
func TestDecodeAndEncodeRoundTrip(t *testing.T) {
	original := []byte{0x00, 0x00, 0x12, 0x34}
	...
	if val.Estimate != 0x1234 {
		t.Errorf("Estimate = %#x, want 0x1234", val.Estimate)
	}
...
func TestCorpusRoundTrip(t *testing.T) {
	...
	if total == 0 {
		t.Skip("no FPSE sections in corpus")
	}
	t.Logf("exercised %d FPSE section(s)", total)
}
