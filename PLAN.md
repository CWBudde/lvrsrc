...
- **Front-panel type-aware layer joins each FPHb data object to its VCTP type (completed).** The same TopTypes indirection carries to the front panel: every front-panel control/indicator/tunnel declares its type with an `OF__typeDesc` (tag 283) leaf in the FPHb heap whose content is a 0-based top-types ordinal — `TopTypes[content]` is the flat VCTP index. This is the panel mirror of the block-diagram constant join (the diagram resolves a constant's type from the nearest-preceding typeDesc in its enclosing object; the panel declares each object's type with its own leaf). Verified independent of the diagram: `Numeric42.vi`'s panel indicator resolves to `NumFloat64` (content `0x02 → TopTypes[2]=3 → VCTP[3]`, a non-identity hop) even though its wired diagram constant is an `I32`; the `_I8`/`_SGL`/`_CDB` controls resolve to `NumInt8`/`NumFloat32`/`NumComplex128`. `pkg/lvvi.Model.FrontPanelTypes` returns one resolved entry per typeDesc leaf in panel order. **Control _default values_ are not decoded** — there is no `DFDS` block anywhere in the corpus and zero `OF__ConstValue` in any FPHb; the only defaults stored are 5 `OF__DefaultData` (tag 571) leaves, all **composite cluster** blobs (e.g. `response.ctl` → version+`"ok"`+…), whose object-level type does not resolve cleanly through the ordinal (it points at a `Void`/placeholder wrapper, not the cluster type). Decoding those needs a recursive VCTP cluster flatten/unflatten walk; `FrontPanelDefaults` surfaces the raw bytes verbatim without inventing a type. A focused fixture — a VI with a single non-zero-default numeric control — would be needed to ground a scalar front-panel default decode. Pinned by `TestFrontPanelTypeJoin`, `…TopTypesIndirection`, `…DefaultsRaw`, and `…NoPanel`; see `fp-control-type-join-via-typedesc` in `reverse-deltas.json`.
