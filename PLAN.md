...
- **`OF__ConstValue` (tag 589) is a fixed-width big-endian literal whose width tracks the constant's data type.** Unlike the compressed-wire byte-stream, a numeric constant is written verbatim in its native type, with no varint and no `0xff` escape. Confirmed across the full representation span by the `controlled-constant` fixtures: I8/U8 = 1 raw byte (`Numeric42_{I8,U8}.vi`, both `2a` — signedness is in the type, not the bytes); I32 = 4-byte BE int (`Numeric{42,255,256,65535,65536}.vi`, straddling the 1- and 2-byte boundaries with no width change); I64 = 8-byte BE int (`Numeric65536_I64.vi`); SGL = 4-byte BE IEEE-754 binary32 (`…_SGL.vi`); DBL = 8-byte BE binary64 (`Numeric9876Dot5432.vi`); EXT = 16-byte BE binary128/quad (`…_EXT.vi`, unbiased exponent 13, _not_ 80-bit x87); FXD = 8-byte BE fixed-point container (`…_FXD.vi`, magnitude `0x26948b` = round(9876.5432·2⁸)); complex = two IEEE-754 components real-then-imaginary, each the underlying float's BE width — CSG = 2×binary32 (8 B, `…_CSG.vi`), CDB = 2×binary64 (16 B, `…_i5_CDB.vi`, 42+5i), CEXT = 2×binary128 (32 B, `…_CEXT.vi`). Width stops being a unique type key past 8 bytes (8 = I64/DBL/CSG/FXD, 16 = EXT/CDB, 32 = CEXT/string), which is why the accessor declines >8-byte leaves and the VCTP type is required. Exposed by the dedicated `pkg/lvvi.HeapConstValue` / `FindConstValueChild` accessors, which decode integer/float widths up to 8 bytes (kept out of the generic scalar set because string/array constants — and 16-byte EXT — exceed 8 bytes and need the VCTP type to disambiguate). Pinned by `TestHeapConstValueGroundTruth`, `TestHeapConstValueFloatGroundTruth`, and `TestHeapConstValueExtendedAndFixedPoint`.

- **Type-aware constant decode joins `OF__ConstValue` to its VCTP type (completed).** The leaf alone cannot disambiguate the >8-byte width collisions (16 = EXT vs CDB, 32 = CEXT vs string); the constant's data type is resolved by the **nearest preceding `OF__typeDesc` (tag 283) leaf within the constant's enclosing heap object**, whose content is a **0-based index into the VCTP top-types list** — `TopTypes[content]` is the flat VCTP index. A diagram constant carries two typeDesc references — the wired indicator's type and the constant's own — and the nearest-preceding is the constant's. The TopTypes indirection is essential, not cosmetic: the single-constant fixtures only ever use content `0x03`/`0x04` where `TopTypes[3]=3`, `TopTypes[4]=4` (an identity coincidence), so `Add17Plus25.vi` (two I32 constants, only 5 VCTP descriptors, `OF__typeDesc=0x07`/`0x09` indexing an 11-entry TopTypes → flat[4]) was the disambiguator that forced the correct scheme — indexing flat VCTP with `0x07`/`0x09` would be out of range. `Numeric42.vi` shows the indicator/constant split (`0x03 → flat[3]=NumFloat64` indicator vs `0x04 → flat[4]=NumInt32` constant); `module-timeout--constant.vi` confirms a non-identity mapping (`TopTypes[4]=1 → flat[1]`). All 71 block-diagram constants in the corpus resolve a VCTP type this way. `pkg/lvvi.Model.BlockDiagramConstants` decodes every constant to a typed value — integers (signed/unsigned per type), SGL/DBL/EXT floats (EXT via a binary128→float64 decoder), CSG/CDB/CEXT complex components real-then-imaginary, and the FXD magnitude — including the 16/32-byte EXT/CDB/CEXT forms `HeapConstValue` declines; non-fixed-width constants (string/array/path/cluster/variant) resolve their type but keep `Kind=unknown`/`WidthMatch=false` rather than guess. Two VCTP type codes were added (`NumComplexExt` 0x0E, `FixedPoint` 0x5F). Pinned by `TestBlockDiagramConstantTypeJoin`, `…TopTypesIndirection`, `…Integers`, `…Floats`, `…Complex`, and `…FixedPoint`; see `const-type-join-via-typedesc` in `reverse-deltas.json`. The **Boolean constant literal is dual-width** — TRUE is a 1-byte `0x01`, FALSE a 2-byte `0x0000`, across all 5 corpus Boolean constants (`BoolToLED.vi`/`format-string.vi` TRUE; `WhileLoop_Numeric42.vi`/`reference-find-by-id.vi` FALSE); the VCTP descriptor (FullType **and** Flags) is identical for both widths and the enclosing DDO object shape is the same, so the width cannot be read from the type, and the value decode (any non-zero byte → true) is width-independent. `constWidthMatches` accepts 1 or 2 bytes for Boolean (pinned by `TestBlockDiagramConstantBoolean`; see `bd-boolean-constant-dual-width` in `reverse-deltas.json`); _why_ FALSE takes the extra byte is not yet explained — only the widths are corpus-confirmed. The remaining `WidthMatch=false` constants are honest gaps, not bugs: the `String`/`LVVariant` blobs are genuinely variable-width (a separate string/variant decode), `Type(0x16)` is an unmodeled VCTP type code, and the lone 9-byte all-zero DBL in `ndjson-parser.vi` is an anomaly that cannot be disambiguated from zeros alone (its value still decodes to 0.0). The **FXP scale is now read from the type config** rather than assumed: a `FixedPoint` (TD 0x5F) constant resolves its VCTP type, whose inner config carries the word length (U16 @ off 2) and integer word length (U32 @ off 4, the bits left of the binary point); the **radix = wordLength − integerWordLength** and the value is `signExtend(mantissa) / 2^radix`. `Numeric9876Dot5432_FXD.vi`'s constant binds to the 64/32 type → radix 32 → mantissa `0x000026948b000000` / 2³² = **9876.54296875** (the FXP quantisation of 9876.5432, exact). Field positions are pinned by cross-check fields (`wordLength−1` @ off 24, `intWordLength−1` @ off 28) and by off-2 matching the constant's container width. `TypedConst` exposes `FixedWordLength`/`FixedIntWordLength`/`FixedRadix`/`FixedValue`/`FixedConfigOK` (the mantissa is sign-extended from the `WL` bits via `signExtendBits`, since the container is 8 bytes even when `WL < 64`); pinned by `TestBlockDiagramConstantFixedPoint`; see `fxp-radix-from-type-config` in `reverse-deltas.json`. The formula is **confirmed** by the asymmetric fixture `Numeric9876Dot5432_FXD_w32i8.vi`: both FXP types in the 64/32 fixture have `wordLength = 2 × integerWordLength` (so `radix = WL − IWL` and `radix = IWL` coincide at 32), but the w32i8 constant binds to an `WL=32`/`IWL=8` type — `radix = WL − IWL = 24` → mantissa `0x2a1f9724` / 2²⁴ = **42.1234** (the entered value), whereas `radix = IWL = 8` would give 2760599.14, decisively refuting it. (Negative-mantissa FXP is not yet corpus-verified — both constants are positive.) The recursive VCTP cluster flatten/unflatten walk for composite defaults is now **completed** (see the front-panel bullet below).

- **Front-panel type-aware layer joins each FPHb data object to its VCTP type (completed).** The same TopTypes indirection carries to the front panel: every front-panel control/indicator/tunnel declares its type with an `OF__typeDesc` (tag 283) leaf in the FPHb heap whose content is a 0-based top-types ordinal — `TopTypes[content]` is the flat VCTP index. This is the panel mirror of the block-diagram constant join (the diagram resolves a constant's type from the nearest-preceding typeDesc in its enclosing object; the panel declares each object's type with its own leaf). Verified independent of the diagram: `Numeric42.vi`'s panel indicator resolves to `NumFloat64` (content `0x02 → TopTypes[2]=3 → VCTP[3]`, a non-identity hop) even though its wired diagram constant is an `I32`; the `_I8`/`_SGL`/`_CDB` controls resolve to `NumInt8`/`NumFloat32`/`NumComplex128`. `pkg/lvvi.Model.FrontPanelTypes` returns one resolved entry per typeDesc leaf in panel order. **Scalar control _default values_ decode through the very same join** (`pkg/lvvi.Model.FrontPanelDefaults`, reusing the block-diagram `resolveConstTypeIndex` + `fillTypedConst`): a default is an `OF__DefaultData` (tag 571) leaf governed by the nearest-preceding typeDesc in its enclosing object. `NumericDblInput.vi` pins it — a DBL control whose current value is committed as the default (LabVIEW _Make Current Value Default_) stores an 8-byte `OF__DefaultData` (`40 c3 4a 45 87 93 dd 98` = 9876.5432) that resolves to `NumFloat64` and decodes exactly; a control left at the zero default stores **no** `OF__DefaultData` (the value 0.0 is never written — there is no `DFDS` block anywhere in the corpus and zero `OF__ConstValue` in any FPHb). The other 5 `OF__DefaultData` leaves are **composite cluster** blobs, now **fully decoded** by a recursive VCTP cluster flatten/unflatten walk (`pkg/lvvi.flatvalue.go`). A composite default is LabVIEW _flattened data_: cluster members serialised back-to-back in declaration order, scalars big-endian fixed-width, strings 4-byte-length-prefixed, **nested clusters recursive**, a trailing `LVVariant` kept opaque. The governing cluster type cannot come from the panel DDO — that object's `OF__typeDesc` children reference only LabVIEW's internal transaction fields (`flg`/`oRt`/`eof`/`udf`/`txd`), never the user data cluster — so it is recovered **structurally**: among every VCTP `Cluster` type, the ones whose flat layout consumes the blob _exactly_ are candidates (exact byte-consumption across nested length-prefixed strings is a very tight constraint), and the best-labelled candidate wins the tie-break (where >1 matches, the extra is LabVIEW's `txd` twin that decodes to the identical value). All 5 decode cleanly: `response.ctl` → `{id:"", status:"ok", result:LVVariant}`; `request.ctl` and `ndjson-parser.vi` (which embeds `Request.ctl` as a TypeDef) → `{id:"", version:1, command:"", params:LVVariant}`; `error-response.ctl` → `{error:{code:0, message:"", details:""}, status:"error", id:""}` (the nested cluster proves the recursion); `show-panel-argument--cluster.ctl` → `{Show Panel?: true}`. `Model.UnflattenValue`/`UnflattenValueN` decode against an explicit flat type index (the principled entry point); `FrontPanelDefaults` populates `TypedConst.Composite`/`CompositeOK`/`CompositeTypeIndex` via `resolveCompositeDefault` for any leaf that is not a clean scalar — the scalar `WidthMatch` stays false (the blob is not a fixed-width scalar), so callers read the structured value from `Composite`. Pinned by `TestFrontPanelTypeJoin`, `…TopTypesIndirection`, `…DefaultScalar`, `…DefaultComposite`, `…NoPanel`, `TestFrontPanelDefaultCompositeDecode`, and `TestUnflattenValueExplicitType`; see `fp-control-type-join-via-typedesc` and `fp-composite-default-cluster-unflatten` in `reverse-deltas.json`.
...
- [x] Add a generic typed accessor for point/size-pair heap fields. `pkg/lvvi.HeapPoint`, `HeapPointForTag`, `FindPointChild`, and `IsHeapPointTag` now expose `HeapNodePoint`-style 4-byte coordinate/size pairs such as `OF__origin`, `OF__minPaneSize`, and `OF__MinButSize` while leaving coordinate origin, size role, and UI effect semantics open.
- [x] Add generic typed accessors for common scalar and color heap fields. `pkg/lvvi.HeapScalar`, `HeapScalarForTag`, `FindScalarChild`, `HeapColor`, `HeapColorForTag`, `FindColorChild`, `IsHeapScalarTag`, and `IsHeapColorTag` now expose common integer / flag / id / count leaves and 4-byte color-like leaves while leaving role-specific bit names and color-prefix semantics open. The scalar registry now also covers low-frequency flag/enum leaves such as `OF__paneFlags`, `OF__MouseWheelSupport`, `OF__annexDDOFlag`, `OF__index`, and fixed-point/FPGA/tunnel enum fields where corpus payloads fit the same bounded integer shape.
- [x] Add generic typed accessors for structural heap container/list fields. `pkg/lvvi.HeapContainer`, `HeapContainerForTag`, `FindContainerChild`, and `IsHeapContainerTag` now expose known open-scope child-list containers such as `OF__image`, `OF__nodeList`, `OF__signalList`, `OF__termList`, and related list tags while leaving child ordering and required/optional member semantics open.
