...
- [ ] Parse primary header (magic, version, data offset, data size, rsrc offset, rsrc size)
- [ ] Parse duplicated/secondary header and validate consistency
- [ ] Parse block info list (type, count, offset)
- [ ] Parse block headers (FourCC, ID, name index)
- [ ] Parse section descriptors (index, offset, size)
- [ ] Parse section payloads (raw bytes)
- [ ] Parse name table / trailing Pascal strings
- [ ] Preserve `RawTail` bytes exactly
- [ ] Add `CompressionKind` detection stub
- [ ] Add `FileKind` detection (`.vi`, `.ctl`, `.vit`, `.llb`)
- [ ] Write parse tests against corpus files
...
## Phase 5 — Typed Resource Expansion *(ongoing)*
